...
lmax       = 2500 # maximum multipole for T, E, B and \phi.
nx         = 512  # number of pixels for flat-sky calc.
dx         = 1./60./180.*np.pi # pixel width in radians.

nlev_p1     = 109.4 # polarization noise level, in uK.arcmin.
nlev_p2     = 81.3

nlev_t1     = 68.1 # polarization noise level, in uK.arcmin.
nlev_t2     = 42.6
...
beam_fwhm1  = 10.0  # Gaussian beam full-width-at-half-maximum.
beam_fwhm2  = 7.1

fsky       = 0.57


data=np.loadtxt('data2500.txt')
...
sldd       = data[:,5]
sldt       = data[:,6]
f1         = data[:,7]  #95 GHZ 1/f noise parameter for EE
f2         = data[:,8]  #150 GHZ 1/f noise parameter for EE
fg1         = data[:,9]  #foreground dust power law for TT
fg2         = data[:,10]  #foreground dust power law for EE
fg3         = data[:,11]  #foreground dust power law for BB
...
nltt1      = (np.pi/180./60.*nlev_t1)**2 / bl1**2
nlee2      = (np.pi/180./60.*nlev_p2)**2 / bl2**2
nltt2      = (np.pi/180./60.*nlev_t2)**2 / bl2**2
